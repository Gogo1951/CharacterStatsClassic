...
function CSC_GetTalentInfo(tabIndex, talentIndex)
    if not g_TalentsIndexMap then
        return GetTalentInfo(tabIndex, talentIndex)
    end

    local tabMap = g_TalentsIndexMap[tabIndex]
    if not tabMap then
        return GetTalentInfo(tabIndex, talentIndex)
    end

    local newTalentIndex = tabMap[talentIndex]
    if not newTalentIndex then
        return GetTalentInfo(tabIndex, talentIndex)
    end
...
function CSC_GetMageCritStatsFromTalents()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(1, 15)) or 0
    ...
    spellRank = select(5, CSC_GetTalentInfo(2, 13)) or 0
    if (spellRank > 0) and (spellRank <= 3) then
...
function CSC_GetMageSpellHitFromTalents()
    ...
    -- Arcane Focus
    local spellRank = select(5, CSC_GetTalentInfo(1, 2)) or 0
    arcaneHit = spellRank * 2 -- 2% for each point

    -- Elemental Precision
    spellRank = select(5, CSC_GetTalentInfo(3, 3)) or 0
...
function CSC_GetWarlockSpellHitFromTalents()
    ...
    -- Suppression
    local spellRank = select(5, CSC_GetTalentInfo(1, 1)) or 0
...
function CSC_GetWarlockCritStatsFromTalents()
    ...
    local devastationCrit = select(5, CSC_GetTalentInfo(3, 7)) or 0
...
function CSC_GetPriestBonusHealingModifierFromTalents()
    -- Spiritual Healing
    local spellRank = select(5, CSC_GetTalentInfo(2, 15)) or 0
...
function CSC_GetPaladinCritStatsFromTalents()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(1, 13)) or 0
...
local function CSC_GetPaladinDefenseFromTalents()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(2, 9)) or 0
    if (spellRank > 0) and (spellRank <= 5) then
...
function CSC_GetPaladinImprovedBoWModifier()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(1, 10)) or 0
...
local function CSC_GetWarriorDefenseFromTalents()
    ...
    -- Anticipation (2, 4, 6, 8, 10)%
    local spellRank = select(5, CSC_GetTalentInfo(3, 2)) or 0
    if (spellRank > 0) and (spellRank <= 5) then
        defense = defenseTable[spellRank]
    end
...
function CSC_GetShamanHitFromTalents()
    -- Nature's Guidance
    local spellRank = select(5, CSC_GetTalentInfo(3, 6)) or 0
    return spellRank
end
...
function CSC_GetShamanCallOfThunderCrit()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(1, 8)) or 0
...
function CSC_GetShamanTidalMasteryCrit()
    ...
    local spellRank = select(5, CSC_GetTalentInfo(3, 11)) or 0
    return spellRank
end
...
function CSC_GetMP5ModifierFromTalents(unit)
    ...
    if unitClassId == CSC_PRIEST_CLASS_ID then
        -- Meditation
        spellRank = select(5, CSC_GetTalentInfo(1, 8)) or 0
    elseif unitClassId == CSC_MAGE_CLASS_ID then
        -- Arcane Meditation
        spellRank = select(5, CSC_GetTalentInfo(1, 12)) or 0
    elseif unitClassId == CSC_DRUID_CLASS_ID then
        -- Reflection
        spellRank = select(5, CSC_GetTalentInfo(3, 6)) or 0
    end
...
function CSC_GetMP5ModifierFromSetBonus(unit)
    ...
    for itemSlot = firstItemslotIndex, lastItemslotIndex do
        ...
        if (itemId) then
            if (itemId == g_VestmentsOfTranscendenceIds[itemId] or 
                itemId == g_StormrageRaimentIds[itemId]) then
